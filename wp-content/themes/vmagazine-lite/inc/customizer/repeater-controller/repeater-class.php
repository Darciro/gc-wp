    ...
    */
    class vmagazine_lite_Repeater_Controler extends WP_Customize_Control {
    	...
    	*/
    	...
    	public $vmagazine_box_label = '';

    	public $vmagazine_box_add_control = '';
    	...
    	public function __construct( $manager, $id, $args = array(), $fields = array() ) {
    		$this->fields = $fields;
    		$this->vmagazine_box_label = $args['vmagazine_box_label'] ;
    		$this->vmagazine_box_add_control = $args['vmagazine_box_add_control'];
    	...
    	public function render_content() {
    		...
    		<ul class="vmagazine-repeater-field-control-wrap">
    			<?php
    			$this->vmagazine_lite_get_fields();
    			?>
    		</ul>

    		<input type="hidden" <?php esc_attr( $this->link() ); ?> class="vmagazine-repeater-collector" value="<?php echo esc_attr( $this->value() ); ?>" />
    		<button type="button" class="button vmagazine-add-control-field"><?php echo esc_html( $this->vmagazine_box_add_control ); ?></button>
    		<?php
    	}
    	...
    	private function vmagazine_lite_get_fields(){
    		...
    		?>
    		<li class="vmagazine-repeater-field-control">
    		<h3 class="vmagazine-repeater-field-title"><?php echo esc_html( $this->vmagazine_box_label ); ?></h3>
    		
    		<div class="vmagazine-repeater-fields">
    		<?php
    			foreach ($fields as $key => $field) {
    			$class = isset($field['class']) ? $field['class'] : '';
    			?>
    			<div class="vmagazine-fields vmagazine-type-<?php echo esc_attr($field['type']).' '.esc_attr($class); ?>">
	    			<?php 
	    				...
	    				switch ($field['type']) {
	    					...
	    					case 'upload':
	    						...
	    						echo '<div class="vmagazine-fields-wrap">';
	    						...
	    						echo '<button type="button" class="button vmagazine-delete-button align-left">'.esc_html__('Remove', 'vmagazine-lite').'</button>';
	    						echo '<button type="button" class="button vmagazine-upload-button alignright">'.esc_html__('Select Image', 'vmagazine-lite').'</button>';
	    					...
	    					case 'colorpicker':
	    						echo '<input data-default="'.esc_attr($default).'" class="vmagazine-color-picker" data-alpha="true" data-name="'.esc_attr($key).'" type="text" value="'.esc_attr($new_value).'"/>';
	    						break;
	    					...
	    					case 'range':
	    						...
	    						echo '<div class="vmagazine-range-slider" >';
	    					...
	    					case 'icon':
	    						echo '<div class="vmagazine-selected-icon">';
	    						echo '<i class="'.esc_attr($new_value).'"></i>';
	    						echo '<span><i class="fa fa-chevron-down"></i></span>';
	    						echo '</div>';
	    						echo '<ul class="vmagazine-icon-list clearfix">';
	    						$vmagazine_icons_array = vmagazine_lite_icons_array();
	    						foreach ($vmagazine_icons_array as $vmagazine_font_awesome_icon) {
	    							$icon_class = $new_value == $vmagazine_font_awesome_icon ? 'icon-active' : '';
	    							echo '<li class='.esc_attr($icon_class).'><i class="fa '.esc_attr($vmagazine_font_awesome_icon).'"></i></li>';
	    						}
	    						echo '</ul>';
	    						echo '<input data-default="'.esc_attr($default).'" type="hidden" value="'.esc_attr($new_value).'" data-name="'.esc_attr($key).'"/>';
	    						break;
	    					...
	    					case 'social_icon':
	    						...
	    						echo '</div>';
	    						echo '<ul class="vmagazine-icon-list clearfix">';
	    						$vmagazine_icons_array = vmagazine_lite_social_icons_arrays();
	    						foreach ($vmagazine_icons_array as $vmagazine_font_awesome_icon) {
	    							$icon_class = $new_value == $vmagazine_font_awesome_icon['icon'] ? 'icon-active' : '';
	    							echo '<li class='.esc_attr($icon_class).'><i class="fa '.esc_attr($vmagazine_font_awesome_icon['icon']).'"></i></li>';
	    						}
	    						echo '</ul>';
	    						echo '<input data-default="'.esc_attr($default).'" type="hidden" value="'.esc_attr($new_value).'" data-name="'.esc_attr($key).'"/>';
	    						break;	
	    					...
	    					case 'multicategory':
	    						...
	    						echo '<ul class="vmagazine-multi-category-list">';
    			...
    			<div class="clearfix vmagazine-repeater-footer">
    				<div class="alignright">
    				<a class="vmagazine-repeater-field-remove" href="#remove"><?php esc_html_e('Delete', 'vmagazine-lite') ?></a> |
    				<a class="vmagazine-repeater-field-close" href="#close"><?php esc_html_e('Close', 'vmagazine-lite') ?></a>
    				</div>
    			</div>
    		</div>
    		</li>
