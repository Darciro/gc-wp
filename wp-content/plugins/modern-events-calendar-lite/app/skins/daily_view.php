...
class MEC_skin_daily_view extends MEC_skins
{
	...
	$semanas[0] = "SEG";
        ...
        {
	    $totalChar = strlen(date_i18n('l', $time));
	    $diaSemana = $semanas[date('N', strtotime($date))];
            $time = strtotime($date);
            $labels .= '<div class="mec-daily-view-day '.(count($events) ? 'mec-has-event' : '').'" id="mec_daily_view_day'.$this->id.'_'.date('Ymd', $time).'" data-events-count="'.count($events).'" data-month-id="'.date('Ym', $time).'" data-day-id="'.date('Ymd', $time).'" data-day-weekday="'.date_i18n('l', $time).'" data-day-monthday="'.date('j', $time).'">'.date_i18n('j', $time)."<div class='dia-agenda'>".$diaSemana.'</div></div>';
        	$contadoDias++;
		if($contadoDias>10){
			return $labels.'</div>';
			break;
