...
<ul class="mec-daily-view-dates-events">
    ...
    <li class="mec-daily-view-date-events mec-util-hidden" id="mec_daily_view_date_events<?php echo $this->id; ?>_<?php echo date('Ymd', strtotime($date)); ?>">
        ...
        <?php foreach($events as $event): ?>
            ...
            <article data-style="<?php echo $label_style; ?>" class="mec-event-article palestras <?php echo $this->get_event_classes($event); ?>">
               <?php /* <?php if(trim($start_time)): ?><div class="mec-event-time mec-color"><i class="mec-sl-clock-o"></i> <?php echo $start_time.(trim($end_time) ? ' - '.$end_time : ''); ?></div><?php endif; ?> */ ?>
				<i class="fa fa-bookmark-o"></i>
				  <div class="txt-palestra">
				...
				<aside> <?php echo date_i18n('d.m.Y', $current_month_time); ?> 
				<a class="bt-1" data-event-id="<?php echo $event->data->ID; ?>" href="<?php echo $this->main->get_event_date_permalink($event->data->permalink, $event->date['start']['date']); ?>">Veja</a>
					</aside>
				</div>
				<img src="<?php echo ($event->data->featured_image['full']) ? esc_html($event->data->featured_image['full']) : '' ; ?>" class="corner" />
					  
