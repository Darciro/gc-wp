...
class SSP_Frontend {
	 ...
	 */
        public function slider_podcast_episode ( $episode_id = 0, $content_items = array( 'title', 'player', 'details' ), $context = '', $style = 'mini' ) {
		$pod[0] = 363;
                $pod[1] = 300;
                $pod[2] = 362;
		$html = '<div class="glider-contain"><div class="glider-podcast">';
		$xp=0;
		while($pod[$xp]){
                	$tempConteudo = $this->pegar_podcast_episode($pod[$xp],$content_items,$context,$style);
			$imgTest = get_the_post_thumbnail( $pod[$xp], apply_filters( 'ssp_frontend_context_thumbnail_size', 'large' ));
			$html = $html."<div Style='width: 450px;'>".$tempConteudo."</div>";
			$html = str_replace('<div class="episode-content"></div>',$imgTest,$html);
                        $xp++;
		}
		$html = $html.'<button role="button" aria-label="Previous" class="glider-prev">«</button><button role="button" aria-label="Next" class="glider-next">»</button>';
		$html = $html.'</div><div role="tablist" class="dots"></div></div>';
		//echo $html;
		return $html;
	}
	...
	public function podcast_episode( $episode_id = 0, $content_items = array( 'title', 'player', 'details' ), $context = '', $style = 'mini' ) {
						...
						switch( $item ) {
							...
							case 'title':
								$html .= '<h3 class="episode-title">' . get_the_title() . '</h3>' . "\n";
								 echo apply_filters( 'ssp_album_art_cover', $albumArt['src'], get_the_ID() );
		...
		$html .= '</div>' . "\n";
		$imgTest = get_the_post_thumbnail( $episode_id, apply_filters( 'ssp_frontend_context_thumbnail_size', 'large' ));
		$html = str_replace('<div class="episode-content"></div>',$imgTest,$html);
		return $html;
	}
